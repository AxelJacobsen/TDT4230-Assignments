...
SceneNode* ballNode;
SceneNode* padNode;
SceneNode* pointLight1;
SceneNode* pointLight2;
SceneNode* pointLight3;
...
void initGame(GLFWwindow* window, CommandLineOptions gameOptions) {
    ...
    // Generate Lights
    pointLight1 = createSceneNode();
    pointLight1->nodeType = POINT_LIGHT;
    pointLight1->rotation = glm::vec3(0.7, 0.0, 0.0);

    pointLight2 = createSceneNode();
    pointLight2->nodeType = POINT_LIGHT;
    pointLight2->rotation = glm::vec3(0.0,0.7,0.0);

    pointLight3 = createSceneNode();
    pointLight3->nodeType = POINT_LIGHT;
    pointLight3->rotation = glm::vec3(0.0,0.0,0.7);
    ...
    rootNode->children.push_back(pointLight1);
    padNode->children.push_back(pointLight2);
    ballNode->children.push_back(pointLight3);
...
void updateFrame(GLFWwindow* window) {
    ...
    glUniformMatrix4fv(3, 1, GL_FALSE, glm::value_ptr(cameraTransform));
    ...
    glUniform3fv(6, 1, glm::value_ptr(ballNode->position));
    ...
    glm::vec3 offset = glm::vec3(0.0, 10.0, 0.0);
    glm::vec3 ballOffset = glm::vec3(0.0, 0.0, 4.0);


    pointLight1->position = glm::vec3(
        boxNode->position.x,
        boxNode->position.y,
        boxNode->position.z);

    pointLight2->position = padNode->position + offset;
    pointLight3->position = ballNode->position + ballOffset;

    for (int o = 0; o < 3; o++) {
        GLint lightPos =
            shader->getUniformFromName(fmt::format("lights[{}].pos", o));
        switch (o) {
        case 0: glUniform4fv(lightPos, 1, glm::value_ptr(glm::vec4(pointLight1->position, 1.0))); break;
        case 1: glUniform4fv(lightPos, 1, glm::value_ptr(glm::vec4(pointLight2->position, 1.0))); break;
        case 2: glUniform4fv(lightPos, 1, glm::value_ptr(glm::vec4(pointLight3->position, 1.0))); break;
        }
    }

    glm::mat4 emptymat = glm::mat4();
    updateNodeTransformations(rootNode,emptymat);
}
...
void updateNodeTransformations(SceneNode* node, glm::mat4 transformationThusFar) {
    ...
    if (transformationThusFar == glm::mat4()) {
        node->currentTransformationMatrix = transformationMatrix;
    }
    else {
    ...
    switch(node->nodeType) {
        ...
        case POINT_LIGHT: {
            GLint lightColor =
                shader->getUniformFromName(fmt::format("lights[{}].color", currentLight));

            glUniform4fv(lightColor, 1, glm::value_ptr(glm::vec4(node->rotation, 1.0)));
            currentLight++;
            break;
        }
...
void renderNode(SceneNode* node) {
    glUniformMatrix4fv(5, 1, GL_FALSE, glm::value_ptr(node->currentTransformationMatrix));
